...
class HomeController extends Controller
{
    ...
    public function search(Request $request)
    {
        Paginator::useBootstrap();
        if ($request->input()) {
            $where = 'services.status = 1 AND services.approved = 1';
            ...
            if ($request->input('location') != '') {
                ...
                $where .= " AND services.location = {$city}";
            }

            $result = Service::select('services.*', 'categories.category_name', 'categories.category_slug')
                ->leftJoin('categories', 'categories.cat_id', '=', 'services.category')
                ->whereRaw($where)
                ->paginate();
            // return $result;
